import React from 'react';
import './Menu.css'; 

const Menu = ( ) => {
return(
<>
   <div className='first-container'>
        <div className='first-text'>
            <h1>Where every <br /> ingredient <br /> tells a story
            </h1>

            <a href="button.com" class="btn-read-more">READ MORE</a>


        </div>

    </div>  

    <div class="second-container">
    <div class="food-card">
      <img src="https://assets.nicepagecdn.com/11a8ddce/4267640/images/dietary-menu-healthy-vegan-salad-vegetables-broccoli-mushrooms-spinach-quinoa-bowl-flat-lay-top-view.jpg" alt="Food Sourcing"/>
      <div class="overlay">
        <h2>FOOD SOURCING</h2>
      </div>
    </div>

    <div class="food-card">
      <img src="https://assets.nicepagecdn.com/11a8ddce/4267640/images/pexels-photo-3850972.jpeg" alt="Menu Development"/>
      <div class="overlay">
        <h2>MENU DEVELOPMENT</h2>
      </div>
    </div>
    <div class="food-card">
      <img src="https://assets.nicepagecdn.com/11a8ddce/4267640/images/italian-spaghetti-with-tomato-sauce-pan.jpg" alt="Restaurant"/>
      <div class="overlay">
        <h2>RESTAURANT</h2>
      </div>
    </div>
  </div>



  <div class="third-container">
    <div class="dish-info">
      <h1>$27</h1>
      <h2>BEEF WITH GINGER</h2>
      <p>Article evident arrived express highest men did boy. Mistress sensible entirely am so. Quick can manor smart money hopes worth too. Comfort produce husband boy her had hearing. Law others theirs passed but wishes. You day real less till dear read. Considered use dispatched melancholy sympathize discretion led. Oh feel if up to till like.</p>
    </div>
    <div class="dish-image">
      <img src="https://assets.nicepagecdn.com/11a8ddce/4267640/images/juicy-steak-medium-rare-beef-with-spices-grilled-vegetables.jpg" alt="Beef with Ginger"/>
    </div>
  </div>



  <div class="second-container">
    <div class="food-card">
      <img src="https://assets.nicepagecdn.com/11a8ddce/4267640/images/penne-pasta-tomato-sauce-with-chicken-tomatoes-wooden-table.jpg" alt="Food Sourcing"/>
      <div class="overlay">
        <h2>FOOD SOURCING</h2>
      </div>
    </div>

    <div class="food-card">
      <img src="https://assets.nicepagecdn.com/11a8ddce/4267640/images/juicy-steak-medium-rare-beef-with-spices-grilled-vegetables.jpg" alt="Menu Development"/>
      <div class="overlay">
        <h2>MENU DEVELOPMENT</h2>
      </div>
    </div>
    <div class="food-card">
      <img src="https://assets.nicepagecdn.com/11a8ddce/4267640/images/pexels-photo-3850972.jpeg" alt="Restaurant"/>
      <div class="overlay">
        <h2>RESTAURANT</h2>
      </div>
    </div>
  </div>



  <div class="fourth-container">
  <div class="dish-image">
      <img src="https://assets.nicepagecdn.com/11a8ddce/4267640/images/juicy-steak-medium-rare-beef-with-spices-grilled-vegetables.jpg" alt="Beef with Ginger"/>
    </div>
    <div class="dish-info">
      <h1>$27</h1>
      <h2>BEEF WITH GINGER</h2>
      <p>Article evident arrived express highest men did boy. Mistress sensible entirely am so. Quick can manor smart money hopes worth too. Comfort produce husband boy her had hearing. Law others theirs passed but wishes. You day real less till dear read. Considered use dispatched melancholy sympathize discretion led. Oh feel if up to till like.</p>
    </div>
    
  </div>
    
    
  <div class="fifth-container">
        <div class="food-item">
            <img src="https://assets.nicepagecdn.com/11a8ddce/4267640/images/high-angle-shot-vegetable-salad-white-bowl.jpg" alt="Hot Pizza"/>
            <h2>HOT PIZZA</h2>
            <button class="read-more">READ MORE</button>
        </div>

        <div class="food-item">
            <img src="https://assets.nicepagecdn.com/11a8ddce/4267640/images/delicious-pizza-isolated-white.jpg" alt="Salads"/>
            <h2>SALADS</h2>
            <button class="read-more">READ MORE</button>
        </div>
    </div>


    <div class="contact-section">
        <div class="contact-item">
            <img src="data:image/png;base64," alt="Email Icon" class="contact-icon"/>
            <h3>CHART TO US</h3>
            <p>Our friendly team is here to help.</p>
            <p class="contact-info">hi@ourcompany.com</p>
        </div>

        <div class="contact-item">
            <img src="data:image/png;base64," alt="Location Icon" class="contact-icon"/>
            <h3>OFFICE</h3>
            <p>Come say hello at our office HQ.</p>
            <p class="contact-info">121 Rock Street, 21 Avenue,<br /> Abbottabad, NY 92103-9000</p>
        </div>

        <div class="contact-item">
            <img src="data:image/png;base64," alt="Phone Icon" class="contact-icon"/>
            <h3>PHONE</h3>
            <p>Mon-Fri from 8am to 5am</p>
            <p class="contact-info">+1(555) 000-000</p>
        </div>
    </div>


    </>
)

}

export default Menu;



